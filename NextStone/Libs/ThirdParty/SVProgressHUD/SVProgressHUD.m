...
+ (void)show {
    if (![SVProgressHUD tempView])
        [[UIApplication sharedApplication] keyWindow].userInteractionEnabled = false;
    [self showWithStatus:nil];
}

+ (void)dismissTempView {
    [[SVProgressHUD tempView] removeFromSuperview];
}

+ (UIView *)tempView {
    return [[[UIApplication sharedApplication] keyWindow] viewWithTag:12311];
}

+ (void)showTempButton {
    [[UIApplication sharedApplication] keyWindow].userInteractionEnabled = true;
    UIButton *tempButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [tempButton setFrame:[UIScreen mainScreen].bounds];
    tempButton.tag = 12311;
    [tempButton addTarget:[SVProgressHUD class] action:@selector(tempButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    [[[UIApplication sharedApplication] keyWindow] addSubview:tempButton];
}

+ (void)tempButtonClicked:(id)sender {
